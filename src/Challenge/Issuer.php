 ...
 * is minted with a `memory` cost (see {@see SilentChallenge}). The nonce is a signed,
...
final class Issuer
{
     ...
     * solution attempt fills and randomly reads it (see {@see SilentChallenge::romix()}).
    ...
    public const ALGORITHM = SilentChallenge::ALGORITHM_SHA256;
    ...
    public function issue(Context $context, int $difficulty = self::DIFFICULTY_DEFAULT, ?int $clearanceTtl = null, int $memory = 0): array
    {
        ...
        return [
            ...
            'algorithm' => SilentChallenge::algorithm($memory),
            'memory' => $memory,
            'expiresAt' => $expiresAt,
